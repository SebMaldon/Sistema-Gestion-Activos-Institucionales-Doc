...
3. **`src/entities/` (Capa de Modelado de Datos ORM):**  
   Contiene las **40 clases de entidad** mapeadas 1:1 o relacionalmente con las tablas del motor SQL Server mediante decoradores de TypeORM (`@Entity`, `@PrimaryColumn`, `@Column`, `@ManyToOne`, `@OneToMany`). Define las restricciones de nulidad, tipos SQL nativos (`uniqueidentifier`, `varchar`, `decimal`, `bit`, `datetime`) y relaciones bidireccionales. Incluye `ArticuloSalidaBienAntiguo.ts` y `SalidaBienAntiguo.ts` para la gestión del historial de salidas heredadas.
...
5. **`src/graphql/resolvers/` (Capa de Controladores y Lógica de Negocio):**  
   Estructurados por dominio institucional (`bienes`, `usuarios`, `auth`, `incidencias/transaccionales`, `salidas`, `salidasAntiguas`, `prestamos`, `solicitudesCambio`, etc.). Reciben las peticiones GraphQL, validan autorización y formato, gestionan transacciones SQL complejas con `EntityManager` y retornan las estructuras tipadas esperadas. `salidasAntiguas.resolver.ts` expone las queries de consulta sobre el historial de salidas heredadas (`SalidaBienAntiguo`, `ArticuloSalidaBienAntiguo`).
...
| `nombre_rol` | `varchar(50)` | NOT NULL | `UNIQUE NONCLUSTERED` | Nombre del rol evaluado en los middlewares. Roles activos: `Maestro` (id=1), `Administrador` (id=2), `Usuario Estándar` (id=3), `Sin Acceso` (id=4). |
...
| `usuario_aprobador_id` | `int` | NULL | `FK` -> `Usuarios(id_usuario)` (`fk_solicitud_aprobador`) | Administrador o Maestro que autoriza el cambio o rechaza la petición en el sistema. |
...
| `id_rol` | `int` | NOT NULL | `DEFAULT ((3))`, `FK` -> `Roles(id_rol)` | Rol de seguridad en el sistema (`1`=Maestro, `2`=Admin, `3`=Usuario). |
...
| **Maker-Checker** | `obtenerSolicitudesPendientes` | *Ninguno* (requiere rol Admin/Maestro) | Recupera todas las solicitudes de modificación de activos radicadas por operadores en espera de aprobación superior. |
...
1. **Guarda de Autenticación y Roles (`requireAuth`, `requireRole`)**: Se invoca `requireAuth(context)` para abortar con HTTP 401 si el JWT no existe. En operaciones críticas (`deleteBien`, `createNotificacion`, `resetPasswordAdmin`), se exige pertenencia estricta mediante `requireRole(context, [ROLES.MAESTRO, ROLES.ADMIN])`.
2. **Inyección Dinámica de Aislamiento por Delegación (`clave_zona`)**: Si el usuario autenticado posee el rol operativo local (`USUARIO`, id_rol=3), el resolutor no confía en los parámetros de filtro del cliente. En su lugar, intercepta la construcción del `QueryBuilder` e inyecta un `INNER JOIN` estructural obligatorio contra la tabla del inmueble (`unidades`), forzando que el activo o incidencia pertenezca exclusivamente a la zona geográfica del operador:
   ```typescript
